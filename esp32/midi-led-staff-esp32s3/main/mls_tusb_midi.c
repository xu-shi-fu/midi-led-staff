...
static const char *s_str_desc[5] = {
    ...
    "Xu-Shi-Fu",          // 1: Manufacturer
    "MIDI-LED-STAFF",     // 2: Product
    "123456",             // 3: Serials, should use chip ID
    "MIDI_LED_STAFF",     // 4: MIDI
};
...
{
    MidiKeyState src;
    bool ok = mls_midi_decode_event(buffer, cb, &src);
    if (ok)
    {
        MidiKeyState *dst = mls_midi_get_key_state_cell(src.note);
        if (dst)
        {
    ...
    for (;;)
    {
        while (tud_midi_available())
        {
            ...
            if (read)
            {
                ...
                mls_midi_handle_event_rx(packet, sizeof(packet));
            }
        }
        vTaskDelay(1);
    }
